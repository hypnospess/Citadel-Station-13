...
/obj/item/organ/eyes/snakeeyes
	...
	var/list/hypnoaid_passive = list(10,0,0,0,0,0)
	var/list/hypnoaid_enthrall = list(15, 2.5, -5, 15, 0.05, 0)
...
/obj/item/organ/eyes/snakeeyes/ComponentInitialize()
	. = ..()
	AddElement(/datum/element/hypnotic, "pos-snakeeyes-enthrall", hypnoaid_enthrall, \
	"The hypnotic eyes pull your attention away.", \
	"As those eyes fade away from your vision, your awareness starts fading back in.", \
	"You can't help but feel nice and relaxed, staring at those eyes.", \
	HYPNOTIC_ABSTRACT)
...
/obj/item/organ/eyes/snakeeyes/Insert(mob/living/carbon/M, special, drop_if_replaced)
	...
	if(!iscarbon(M))
		return
	//Hey since we're no longer relying on hacky code hooking into mkultra enthrallment, this probably isn't necessary
...
/obj/item/organ/eyes/snakeeyes/on_life()
	...
	for(var/mob/living/carbon/M in in_sight)
		if(get_cardinal_dir(owner, M) == owner.dir && turn(M.dir, 180) == owner.dir)
			...
			var/datum/status_effect/hypno/H = M.has_status_effect(/datum/status_effect/hypno)
			if(istype(H))
				H.add_abstract_aid(src)
		else
			in_sight -= M
...
/obj/item/organ/eyes/snakeeyes/proc/do_enthrallment(mob/living/carbon/M)
	...
	var/datum/status_effect/hypno/H = M.has_status_effect(/datum/status_effect/hypno)
	if(!H)
		H = M.apply_status_effect(/datum/status_effect/hypno)
		H.add_abstract_aid(src)
...
/obj/item/organ/eyes/snakeeyes/proc/toggle_spiral(silent)
	if(!has_spiral)
		...
		add_mob_overlay()
		if(!QDELETED(owner))
			owner.AddElement(/datum/element/hypnotic, "pos-snakeeyes-passive", hypnoaid_passive) //A small, passive scalecap boost for everyone, including the 'tist themself.
	else
		if(!silent)
		...
		remove_mob_overlay()
		if(!QDELETED(owner))
			owner.RemoveElement(/datum/element/hypnotic, "pos-snakeeyes-passive", hypnoaid_passive) //This looks wack. And it kinda is.
		for(var/mob/living/carbon/M in mesmerized_subjects)
			mesmerized_subjects -= M
			var/datum/status_effect/hypno/H = M.has_status_effect(/datum/status_effect/hypno)
			if(istype(H))
